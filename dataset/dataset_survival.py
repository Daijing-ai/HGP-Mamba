...
class Generic_WSI_Survival_Dataset(Dataset):
    ...
    def summarize(self):
        ...
        # self.slide_data.to_csv('slide_data_summary.csv', index=False)
        for i in range(self.num_classes):
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        wsi_path = os.path.join(data_dir, 'HNE_features', '{}.pt'.format(slide_id.rstrip('.svs')))
                        # wsi_path = os.path.join(data_dir, f"{slide_id.split('.')[0][:12]}_512.pt" )
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                    path_features = torch.cat(path_features, dim=0)
                    return (path_features, torch.zeros((1,1)), label, event_time, c, slide_id)
                
                elif self.mode == 'mIF':
                    mIF_features = []
                    for slide_id in slide_ids:
                        mIF_path = os.path.join(data_dir, 'mIF_features', '{}.h5'.format(slide_id.rstrip('.svs')))
                        # mIF_bag = h5py.File(mIF_path, 'r')['features'][:]
                        mIF_bag = torch.load(mIF_path)
                        # mIF_features.append(torch.tensor(mIF_bag))
                        mIF_features.append(mIF_bag)
                    mIF_features = torch.cat(mIF_features, dim=0)
                    return (torch.zeros((1,1)), mIF_features, label, event_time, c)
                    ...
                    for slide_id in slide_ids:
                        ...
                        mIF_features.append(bag['mIF'])

                    WSI_features = torch.cat(WSI_features, dim=0) if len(WSI_features) > 0 else torch.zeros((1,1))
                    mIF_features = torch.cat(mIF_features, dim=0) if len(mIF_features) > 0 else torch.zeros((1,1))
                    return (WSI_features, mIF_features, label, event_time, c, slide_id)
