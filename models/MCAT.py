...
class BilinearFusion(nn.Module):
    ...
    def forward(self, vec1, vec2):
        ...
        ### Fusion
        device = o1.device  
        o1 = torch.cat((o1, torch.ones(o1.shape[0], 1, device=device).fill_(1)), 1)
        o2 = torch.cat((o2, torch.ones(o2.shape[0], 1, device=device).fill_(1)), 1)
...
class MCAT_Surv(nn.Module):
    def __init__(self, fusion='bilinear', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
                 model_size_wsi: str='small', model_size_omic: str='small', dropout=0.25, in_dim=1024):
    ...
    def forward(self, x, y):
        ...
        # h_path_bag = self.wsi_net(x).unsqueeze(1) ### path embeddings are fed through a FC layer     ##[N, B, D]
        # h_omic_bag = self.mlp(y).unsqueeze(1)  ### omic embeddings are fed through a snn layer       ##[N, B, D]

        h_path_bag = self.wsi_net(x).unsqueeze(0).unsqueeze(2)   #b, n1, h, d
        h_omic_bag = self.mlp(y).unsqueeze(0).unsqueeze(2)       #b, n2, h, d

        h_omic_bag = h_omic_bag.to(torch.float16)
        h_path_bag = h_path_bag.to(torch.float16)

        # Coattn
        h_path_coattn = flash_attn_func(h_omic_bag, h_path_bag, h_path_bag)     ##[b, n2, h, d]

        h_path_coattn = h_path_coattn.to(torch.float32)       
        h_omic_bag = h_omic_bag.to(torch.float32)

        h_path_coattn = h_path_coattn.squeeze(2)       #b, n2, d
        h_omic_bag = h_omic_bag.squeeze(2)             #b, n2, d

        h_path_coattn = torch.transpose(h_path_coattn, 0, 1)   ##[N, B, D]
        h_omic_bag = torch.transpose(h_omic_bag, 0, 1)       ##[N, B, D]

        # h_path_coattn, A_coattn = self.coattn(h_omic_bag, h_path_bag, h_path_bag)

        ### Path
        h_path_trans = self.path_transformer(h_path_coattn)          ##[N, B, D]
...
if __name__ == '__main__':
    ...
    model = MCAT_Surv(fusion='concat', n_classes=4, model_size_wsi='small', model_size_omic='small', dropout=0.25, in_dim=512).cuda()
    print(model.eval())
